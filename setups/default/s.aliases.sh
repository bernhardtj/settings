...
rpt_fix_rpmfusion() {
    if [[ -x "$HOME/settings/bin/settings-install" ]]; then
        "$HOME/settings/bin/settings-install" default enable-rpmfusion "$@"
    elif [[ -x "$HOME/settings-new/bin/settings-install" ]]; then
        "$HOME/settings-new/bin/settings-install" default enable-rpmfusion "$@"
    else
        echo "rpt_fix_rpmfusion: settings-install not found" >&2
        return 1
    fi
}
